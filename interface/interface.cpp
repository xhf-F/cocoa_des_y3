...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// CLASS PointMass MEMBER FUNCTIONS (& RELATED) - READ MASK, COV..
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
...
arma::Mat<double> cpp_print_C1TA_BTA_C2TT() {
  ...
  for (int nz=0; nz<tomo.shear_Npowerspectra; nz++) {
    for (int j=0; j<Ntable.N_a; j++) {
      ...
      r(nz*Ntable.N_a+j,1) = a;
      //r(nz*Ntable.N_a+j,2) = C1_TA(a,nz);
      //r(nz*Ntable.N_a+j,3) = b_TA(a,nz);
      //r(nz*Ntable.N_a+j,4) = C2_TT(a,nz);
    }
  }
  return r;
}
...
arma::Mat<double> cpp_print_IA() {
  arma::Mat<double> r(FPT.N, 12, arma::fill::zeros);
  if (!(FPT.tab_IA == 0)) {
    std::vector<double> k(FPT.N);
    std::vector<double> Pin(FPT.N);
    FPT_input(k.data(), Pin.data());
    for (int i = 0; i < FPT.N; i++) {
      r(i,0) = k[i];
      r(i,1) = Pin[i];
      r(i,2) = FPT.tab_IA[0][i]; // tt_EE
      r(i,3) = FPT.tab_IA[1][i]; // tt_BB
      r(i,4) = FPT.tab_IA[2][i]; // ta_dE1
      r(i,5) = FPT.tab_IA[3][i]; // ta_dE2
      r(i,6) = FPT.tab_IA[4][i]; // ta_EE
      r(i,7) = FPT.tab_IA[5][i]; // ta_BB
      r(i,8) = FPT.tab_IA[6][i]; // mix_A
      r(i,9) = FPT.tab_IA[7][i]; // mix_B
      r(i,10) = FPT.tab_IA[8][i]; // mix_D_EE
      r(i,11) = FPT.tab_IA[9][i]; // mix_D_BB
    }
  }
  return r;
}
...
PYBIND11_MODULE(cosmolike_interface, m) {
    ...
    m.def("print_cee_cbb", &cpp_print_CEE_CBB, "Print Shear C_EE(l) and C_EE(l)");

    m.def("print_c1ta_bta_c2tt", &cpp_print_C1TA_BTA_C2TT, "Print Shear C1_TA(a,nz), B_TA(a,nz) and C2_TT(a,nz)");

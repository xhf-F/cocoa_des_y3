...
parameter = [u'omegam',u'sigma8',u'omegam_growth',u'DES_A1_1',u'DES_A1_2']
chaindir=r'.'
...
root_chains = ('EXAMPLE_MCMC2','EXAMPLE_MCMC3','EXAMPLE_MCMC12','EXAMPLE_MCMC13',)
...
samples.saveAsText('.VM_P1v2_TMP1')
# --------------------------------------------------------------------------------
...
samples.ranges.setRange('w_growth', [w_min, w_max])
samples.ranges.setRange('H0', [100*h_min, 100*h_max])
samples.saveAsText('.VM_P1v2_TMP2')
# --------------------------------------------------------------------------------
...
samples.ranges.setRange('w_growth', [w_min, w_max])
samples.ranges.setRange('H0', [100*h_min, 100*h_max])
samples.saveAsText('.VM_P1v2_TMP3')
# --------------------------------------------------------------------------------
...
samples.ranges.setRange('w_growth', [w_min, w_max])
samples.ranges.setRange('H0', [100*h_min, 100*h_max])
samples.saveAsText('.VM_P1v2_TMP4')
# --------------------------------------------------------------------------------
...
param_3d = None
g.triangle_plot(['.VM_P1v2_TMP1','.VM_P1v2_TMP2','.VM_P1v2_TMP3', '.VM_P1v2_TMP4'],parameter,
...
for ax in g.subplots[:,3]:
  if ax is not None:
    ax.axvline(0.606102, color='gray', ls='--',alpha=0.3)
for ax in g.subplots[:,4]:
  if ax is not None:
    ax.axvline(-1.51541, color='gray', ls='--',alpha=0.3)
...
for ax in g.subplots[3,0:3]:
  if ax is not None:
    ax.axhline(0.606102, color='gray', ls='--',alpha=0.3)
for ax in g.subplots[4,0:4]:
  if ax is not None:
    ax.axhline(-1.51541, color='gray', ls='--',alpha=0.3)
